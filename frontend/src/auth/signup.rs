...
use common::models::{UserTransmission, DisplayState};
...
pub fn Signup() -> Html {
    ...
    let alert_html = match &*alert_state{
        ...
        DisplayState::Success{message} => html!{<div class="alert alert-success" role="alert">{"Success: "}{message}</div>},
        DisplayState::Failure{message} => html!{<div class="alert alert-danger" role="alert">{"Failure: "}{message}</div>}
    };
    ...
    let onsubmit = Callback::from(move |event: SubmitEvent| {
        ...
        wasm_bindgen_futures::spawn_local(async move {
            ...
            alert_state_cloned.set(match serde_json::from_str(&post_submission){
                Ok(c) => c,
                Err(e) => DisplayState::Failure { message: e.to_string() }
            });
        });
